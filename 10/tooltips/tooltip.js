...
let numStudents = tooltip.append('div')
    .text('# of students: ')
    .classed('tt-text', true)

let studentSpan = numStudents.append('span')
            .classed('tt-text', true)

let bars = inner.selectAll('.course-rect')
bars.on('mouseover', (pointer, data) => {
    tooltip.style('opacity', 1)
    courseSpan.text(data.type)
    studentSpan.text(data.students)
})
...
bars.on('mousemove', function(pointer, data) {
    let w = +d3.select(this).attr('width')
    let h = +d3.select(this).attr('y')
    h += 35
    w += 50
    tooltip.style('left', w+'px')
