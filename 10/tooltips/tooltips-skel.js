...
between*/
let margin = {top:30, right:30, left:30, bottom:60}
...
let yLabel = svg.append('text')
                ...
                .classed('label', true)
                .attr('x', innerWidth/2)
                .attr('y', innerHeight+margin.top+margin.bottom-20)
...
// tooltip pixel space
// aka a div that we'll be moving around the viz
/*
const tooltip = d3.select('body').append('div')
    .attr('id', 'tooltip')
*/
/* attach divs for each value name 
   and spans to hold each value
*/
/*
let courseType = tooltip.append('div')
    .text('course type: ')
    .classed('tt-text', true)

let courseSpan = courseType.append('span')
        .classed('tt-text', true)
*/
// finish with student enrollment stats

/* attach tooltip event to bars –>

   1. select bars
   2. attach event listeners for mouseout and mouseover (USE FUNCTION NOTATION)
   3. style its position based on mousemove

this is where THIS comes in 
if you only need to access the DATA, you can pass in (p, d)
IF you need to access element ATTRIBUTES (x, y, width, height, etc.), select THIS

THIS returns a reference to the object the mouse is interacting with!!
*/

/*
let bars = inner.selectAll('.course-rect')
bars.on('mouseover', (pointer, data) => {
    console.log('hi')
})
